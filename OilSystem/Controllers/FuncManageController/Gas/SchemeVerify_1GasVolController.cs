...
[ApiController]
[Route("[controller]")]
public class SchemeVerify_1GasVolController : ControllerBase
{

    private readonly oilblendContext context;

    public SchemeVerify_1GasVolController(oilblendContext _context)
    {
       context = _context;
    }
    ...
    public ApiModel Set1()//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        ...
        for(int i = 0; i < ProdOilProductList.Count; i++){
            ...
            result.gas92Product = ProdOilProductList[i].gas92VolumeProduct;
            result.gas95Product = ProdOilProductList[i].gas95VolumeProduct;
            result.gas98Product = ProdOilProductList[i].gas98VolumeProduct;
            result.gasSelfProduct = ProdOilProductList[i].gasSelfVolumeProduct;
    ...
    public ApiModel Put1(GasSchemeVerify_1_1_index obj)//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        ...
        ProdOilProductList[obj.index].gas92VolumeProduct = obj.gas92Product;
        ProdOilProductList[obj.index].gas95VolumeProduct = obj.gas95Product;
        ProdOilProductList[obj.index].gas98VolumeProduct = obj.gas98Product;
        ProdOilProductList[obj.index].gasSelfVolumeProduct = obj.gasSelfProduct;
    ...
    public ApiModel Set2()//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        ...
        for(int i = 0; i < BottomInfoList.Count; i++){
            ...
            result.Bottomron = BottomInfoList[i].ronVol;
            result.Bottomt50 = BottomInfoList[i].t50Vol;
            result.Bottomsuf = BottomInfoList[i].sufVol;
    ...
    public ApiModel Put2(GasSchemeVerify_1_2_index obj)//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        ...
        BottomInfoList[obj.index].ronVol = obj.Bottomron;
        BottomInfoList[obj.index].t50Vol = obj.Bottomt50;
        BottomInfoList[obj.index].sufVol = obj.Bottomsuf;
