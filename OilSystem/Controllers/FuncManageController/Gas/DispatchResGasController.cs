...
[ApiController]
[Route("[controller]")]
public class DispatchResGasController : ControllerBase
{
    //智能决策模块设置控制器

    private readonly oilblendContext context;

    public DispatchResGasController(oilblendContext _context)
    {
       context = _context;
    }
    ...
    {       
        ...
        List<GasDispatch_decsScheme_comFlowInfo_divProd> ResultList = new List<GasDispatch_decsScheme_comFlowInfo_divProd>();//列表，里面可以添加很多个对象  
        for(int i = 0; i < list1.Count; i++){//组分油个数
            GasDispatch_decsScheme_comFlowInfo_divProd result = new GasDispatch_decsScheme_comFlowInfo_divProd();//实体，可以理解为一个对象  
