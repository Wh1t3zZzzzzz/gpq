...
[ApiController]
[Route("[controller]")]
public class RecipeCalc_1GasController : ControllerBase
{

    private readonly oilblendContext context;

    public RecipeCalc_1GasController(oilblendContext _context)
    {
       context = _context;
    }
    ...
    {
        var ComOilProductLimitList = context.Recipecalc1_gases.ToList();//
        List<GasRecipecalc_1_1> ResultList = new List<GasRecipecalc_1_1>();//列表，里面可以添加很多个对象
        for(int i = 0; i < ComOilProductLimitList.Count; i++){
            GasRecipecalc_1_1 result = new GasRecipecalc_1_1();//实体，可以理解为一个对象  
    ...
    public ApiModel Put1(GasRecipecalc_1_1_index obj)//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        var ComOilProductLimitList = context.Recipecalc1_gases.ToList();
        ...
        context.Recipecalc1_gases.Update(ComOilProductLimitList[obj.index]);
    ...
    {
        var OptimizeObjList = context.Recipecalc2_gases.Where(m => m.Apply == 1).ToList();
        List<GasRecipecalc_1_2> ResultList = new List<GasRecipecalc_1_2>();//列表，里面可以添加很多个对象
        for(int i = 0; i < OptimizeObjList.Count; i++){
            GasRecipecalc_1_2 result = new GasRecipecalc_1_2();//实体，可以理解为一个对象  
    ...
    public ApiModel Put2(GasRecipecalc_1_2_index obj)//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        var OptimizeObjList = context.Recipecalc2_gases.Where(m => m.Apply == 1).ToList();
        OptimizeObjList[obj.index].Weight = obj.Weight;
        context.Recipecalc2_gases.Update(OptimizeObjList[obj.index]);//是按经过where查询后的list顺序更改的
        context.SaveChanges();   
        return new ApiModel()
        {
        code = 200,
    ...
    {
        var ProdOilProductLimitList = context.Recipecalc3_gases.Where(m => m.Apply == 1).ToList();
        List<GasRecipecalc_1_3> ResultList = new List<GasRecipecalc_1_3>();//列表，里面可以添加很多个对象
        for(int i = 0; i < ProdOilProductLimitList.Count; i++){
            GasRecipecalc_1_3 result = new GasRecipecalc_1_3();//实体，可以理解为一个对象  
    ...
    public ApiModel Put3(GasRecipecalc_1_3_index obj)//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        var ProdOilProductLimitList = context.Recipecalc3_gases.Where(m => m.Apply == 1).ToList();
        ...
        context.Recipecalc3_gases.Update(ProdOilProductLimitList[obj.index]);
    ...
    {
        var ComOilTankVolList = context.Recipecalc1_gases.ToList();
        List<GasRecipecalc_1_4> ResultList = new List<GasRecipecalc_1_4>();//列表，里面可以添加很多个对象
        for(int i = 0; i < ComOilTankVolList.Count; i++){
            GasRecipecalc_1_4 result = new GasRecipecalc_1_4();//实体，可以理解为一个对象  
    ...
    public ApiModel Put4(GasRecipecalc_1_4_index obj)//model里的名字 多个数据用IEnumberable，单个数据不用
    {
        var ComOilTankVolList = context.Recipecalc1_gases.ToList();
        var list1 = context.Schemeverify1_gases.ToList();
        var list2 = context.Compoilconfig_gases.ToList();
        ...
        context.Recipecalc1_gases.Update(ComOilTankVolList[obj.index]);
        context.Schemeverify1_gases.Update(list1[obj.index]);
        context.Compoilconfig_gases.Update(list2[obj.index]);
        context.SaveChanges();
